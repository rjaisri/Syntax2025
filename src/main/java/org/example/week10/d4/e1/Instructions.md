# Tracking Unique Customer Orders Using HashSet
...
1. **Create the `E1Set.java` class** with the following:
...
**Explanation:** If you were to use an `ArrayList`, all customer IDs would be added, including duplicates, which could lead to incorrect data analysis (such as counting total unique customers).
With a `HashSet`, duplicate IDs are automatically removed, ensuring that each customer is counted only once.
